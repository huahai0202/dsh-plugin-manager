...
const MAX_PNPM_OUTPUT_BYTES = 256 * 1024;
...
function runPnpm(args, cwd) {
  return new Promise((settle) => {
    const child = spawn("pnpm", args, { cwd, shell: process.platform === "win32", windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    let finished = false;
    ...
    const capture = (current, data) => {
      ...
      if (next.length <= MAX_PNPM_OUTPUT_BYTES) return next;
      outputTruncated = true;
      return next.slice(-MAX_PNPM_OUTPUT_BYTES);
    };
    const finish = (result) => {
      if (finished) return;
      finished = true;
      clearTimeout(timeout);
      settle({ ...result, stdout, stderr, outputTruncated });
    };
    ...
    const timeout = setTimeout(() => {
      child.kill();
      finish({ code: 124, stderr: `${stderr}\npnpm timed out after ${PNPM_TIMEOUT_MS} ms` });
    }, PNPM_TIMEOUT_MS);
    timeout.unref?.();
  });
}
